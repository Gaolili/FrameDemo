...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color
{
    ...
    if (self) {
        ...
        _secondView = [self creatViewAtPosition:CGPointMake(0, self.frame.origin.y + self.frame.size.height)];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show    complete:(void(^)())complete
{
    if (show) {
        ...
        if (_tapIndex ==2) {
            _secondView.frame =CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
            [self.superview addSubview:_secondView];
            [UIView animateWithDuration:0.2 animations:^{
                _secondView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 100);
            }];
        }else{
...
- (UIView *)creatViewAtPosition:(CGPoint)point{
    UIView * views = [UIView new];
    views.frame =CGRectMake(point.x, point.y, self.frame.size.width, 100);
    return views;
}
